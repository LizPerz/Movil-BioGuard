...
class SensorRepository(
    ...
    suspend fun atenderEvento(eventoId: String, accion: String): Resource<String> {
        return try {
            val response = api.atenderEvento(eventoId, AtenderEventoRequest(notasAtencion = accion))
            Resource.Success(response.message)
        } catch (e: Exception) {
